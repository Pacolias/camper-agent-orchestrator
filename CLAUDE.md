...
* **Multi-leg replanning on infeasibility.** When a leg is infeasible, `supervisor_node` does not just end with a warning — it replans as two legs, without any change to the graph topology in `app/agents/graph.py`. Mechanism: `sql_agent_node`/`rag_agent_node`/`math_agent_node` are generic over whatever `state["origin"]`/`state["destination"]` currently are, so the supervisor repoints those two fields at a sub-leg and clears `poi_data`/`legal_context`/`legal_region`/`fuel_cost`/`math_analysis`, which makes the existing routing rules (rules 1-3 in the prompt) naturally re-trigger the sql→rag→math cycle for that sub-leg — no new graph nodes needed. `legal_context_by_region` is deliberately NOT cleared between legs (see "RAG pipeline" below) — it accumulates across the whole request so a repeated region isn't re-fetched. Concretely, in `supervisor_node`:
